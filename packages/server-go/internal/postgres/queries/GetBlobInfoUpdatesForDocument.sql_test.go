...
func TestGetBlobInfoUpdatesForDocument(t *testing.T) {
	...
	t.Run("sice zero should return blob info", func(t *testing.T) {
		rows, err := q.GetBlobInfoUpdatesForDocument(context.Background(), zeroTime, docUUID)
	...
	t.Run("since inf should return none", func(t *testing.T) {
		rows, err := q.GetBlobInfoUpdatesForDocument(context.Background(), maxTime, docUUID)
		require.NoError(t, err)
